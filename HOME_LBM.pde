...
void setup(){  
  ...
  for(int i=0; i<simulation.getNx() ;i++)
    for(int j=0; j<simulation.getNy() ;j++) {
      if(i==0){
        simulation.setCell(i, j, CELL_TYPE.SOLID, false, 1.f, 0.f, 0.f, 0.f);
      } else if (i==simulation.getNx()-1) {
        simulation.setCell(i, j, CELL_TYPE.SOLID, false, 1.f, 0.f, 0.f, 1.f);  
      }  else {
        simulation.setCell(i, j, CELL_TYPE.LIQUID, false, 1.f, 0.f, 0.f, float(i)>(float(2*GRID_SIZE_Y)+0.1f*float(GRID_SIZE_Y)*cos(2.f*PI*float(j)/float(GRID_SIZE_Y))) ? 1.f : 0.f);
      }
    }
...
void draw(){
  ...
  for(int i=0; i<simulation.getNx() ;i++) {
    for(int j=0; j<simulation.getNy() ;j++) {
      if(simulation.getType(i,j)==CELL_TYPE.SOLID || simulation.isEQ(i,j)) continue;
  ...
  for(int i=(mouseX/SCREEN_ZOOM-radius); i<(mouseX/SCREEN_ZOOM+radius) ;i++)
    for(int j=(mouseY/SCREEN_ZOOM-radius); j<(mouseY/SCREEN_ZOOM+radius) ;j++) {
      if(i>=0 && i<simulation.getNx() && j>=0 && j<simulation.getNy() && inSphere(i,j,radius,mouseX/SCREEN_ZOOM,mouseY/SCREEN_ZOOM))
        if(simulation.getType(i,j)==CELL_TYPE.LIQUID) 
