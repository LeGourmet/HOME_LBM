...
float plic_cube_reduced(float V, float n1, float n2, float n3) { // optimized solution from SZ and Kawano, source: https://doi.org/10.3390/computation10020021
  ...
  if(n12<=2.f*n3V) return n3V+0.5f*n12; // case (5)
  float sqn1=sq(n1), n26=6.f*n2, v1=sqn1/n26; // after case (5) check n2>0 is true
  if(v1<=n3V && n3V<v1+0.5f*(n2-n1)) return 0.5f*(n1+sqrt(sqn1+8.f*n2*(n3V-v1))); // case (2)
  float V6 = n1*n26*n3V;
  if(n3V<v1) return cbrt(V6); // case (1)
  float v3 = n3<n12 ? (sq(n3)*(3.f*n12-n3)+sqn1*(n1-3.f*n3)+sq(n2)*(n2-3.f*n3))/(n1*n26) : 0.5f*n12; // after case (2) check n1>0 is true
...
float calculate_curvature(float[] phij) { // calculate surface curvature, always use D3Q27 stencil here, source: https://doi.org/10.3390/computation10020021
  ...
  for(int i=1; i<9; i++) { // iterate over neighbors, no loop unrolling here (50% better perfoemance without loop unrolling)
    if(phij[i]>0.f && phij[i]<1.f) { // limit neighbors to interface cells
      PVector ei = new PVector(D2Q9_cx[i], D2Q9_cy[i], 0.f); // assume neighbor normal vector is the same as center normal vector
      float offset = plic_cube(phij[i], by)-center_offset;
      p[number++] = new PVector(PVector.dot(ei, bx), PVector.dot(ei, by)+offset, 0.f); // do coordinate system transformation into (x, f(x)) and apply PLIC pffsets
    }
  }
